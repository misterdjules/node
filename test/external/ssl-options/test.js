...
function secureProtocolsCompatible(serverSecureProtocol,
...
function testSetupsCompatible(serverSetup, clientSetup) {
  ...
  var serverIsSsl2 = isSsl2Protocol(serverSetup.secureProtocol);
  var clientIsSsl2 = isSsl2Protocol(clientSetup.secureProtocol);
  if (serverIsSsl2 || clientIsSsl2) {
    /*
     ...
     */
    var serverCiphersSsl2Compatible =
        serverSetup.ciphers === SSL2_COMPATIBLE_CIPHERS;
    var clientCiphersSsl2Compatible =
        clientSetup.ciphers === SSL2_COMPATIBLE_CIPHERS;
    if ((!serverIsSsl2 || !clientIsSsl2) &&
        (!clientCiphersSsl2Compatible || !serverCiphersSsl2Compatible)) {
      return false;
    }
  }
...
function sslSetupMakesSense(cmdLineOption, secureProtocol, secureOption) {
  var cmdLineEnablesSsl2 = cmdLineOption &&
      cmdLineOption.indexOf('--enable-ssl2') !== -1;

  if (isSsl2Protocol(secureProtocol)) {
    if (secureOption & constants.SSL_OP_NO_SSLv2 || !cmdLineEnablesSsl2) {
      return false;
    }
  }

  var cmdLineEnablesSsl3 = cmdLineOption &&
      cmdLineOption.indexOf('--enable-ssl3') !== -1;

  if (isSsl3Protocol(secureProtocol)) {
    if (secureOption & constants.SSL_OP_NO_SSLv3 || !cmdLineEnablesSsl3) {
      return false;
    }
  }
